...
# ------------------------------------------------------------------------------------
# Custom Dataset with Preload Option
# ------------------------------------------------------------------------------------
class DeforestationDataset(Dataset):
    """
    A PyTorch Dataset for reading 4-channel imagery (GeoTIFF).
    When preload=True, it loads all images and masks into memory.
    """
    def __init__(self, image_paths, mask_paths, transform=None, preload=False):
        ...
        if self.preload:
            ...
            print(f"Preloaded {len(self.images)} images and masks.")
...
def get_transforms():
    """
    Albumentations pipeline with scaling for Sentinel-2 imagery,
    resizing, flips, rotations, brightness/contrast, normalization, and tensor conversion.
    """
    return A.Compose([
        A.Resize(512, 512),
...
def main(base_path):
    ...
    train_dataset = DeforestationDataset(train_img_paths, train_mask_paths, transform=get_transforms(), preload=True)
    val_dataset   = DeforestationDataset(val_img_paths,   val_mask_paths,   transform=get_transforms(), preload=True)
