...
from sklearn.metrics import (
    ...
    precision_recall_curve, roc_curve, auc, jaccard_score
)
...
EPOCHS = 230
LEARNING_RATE = 1e-4
ENCODER_NAME = "resnext101_32x8d"  # Replace with your choice
    ...
    def __init__(self, image_paths, mask_paths, transform=None):
        ...
        for img_path, mask_path in tqdm(zip(image_paths, mask_paths), 
            ...
            with rasterio.open(img_path) as img_file:
                image = img_file.read().transpose(1, 2, 0)  # (H, W, C)
                image = (image / 10000.0).astype(np.float32)  # Normalize
            with rasterio.open(mask_path) as mask_file:
                mask = mask_file.read(1).astype(np.float32)  # (H, W)
...
def compute_metrics_per_sample(model, dataset, threshold=0.5, device=DEVICE):
    ...
    for idx in range(len(dataset)):
        ...
        sample_iou = iou_metric(pred.squeeze(1), mask_tensor.unsqueeze(0).int().to(device)).item()
...
def train_model(model, train_loader, val_loader):
    ...
    # Use the default AMP API for your PyTorch version
    ...
    for epoch in range(EPOCHS):
        ...
        for images, masks in train_loader:
            ...
            total_train_iou += iou_metric(outputs.sigmoid().squeeze(1), masks.int())
        ...
        with torch.no_grad():
            for images, masks in val_loader:
                ...
                total_val_iou += iou_metric(outputs.sigmoid().squeeze(1), masks.int())
        ...
        if avg_val_iou > best_iou:
            ...
            torch.save(model.state_dict(), "best_model_unetplusplus_efficientnetb8.pth")
    ...
    with torch.no_grad():
        for images, masks in val_loader:
            ...
            confmat_metric.update(outputs.sigmoid().squeeze(1), masks.int())
...
def compute_probs_and_targets(model, val_loader):
    ...
    with torch.no_grad():
        for images, masks in val_loader:
            ...
            trues = masks.cpu().numpy()                      # shape: (B, 1, H, W) or (B, H, W)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train and evaluate Unet++ on 4-channel data using the second GPU (if available).")
